...
type ShiftType = 'morning' | 'afternoon' | 'night' | 'dayOff';
...
const Schedules: React.FC = () => {
  ...
  const assignAuto = async () => {
    ...
    const updatedSchedulesResponse = await fetch('/api/schedules');
    ...
    <div>
      ...
      >
        ...
        {employees.map(employee => (
          <option key={employee.id} value={`${employee.firstName}.${employee.lastName?.charAt(0)}`}>
      ...
      {editMode.day && editMode.shift && (
        <div>
          <h3>
            Editando {shiftNames[editMode.shift]} del día {editMode.day}
          </h3>
          <Select
            isMulti
            ...
            <tr key={index}>
              ...
              {schedules.map((schedule, scheduleIndex) => (
                <td
                  ...
                  style={{
                    cursor: 'pointer',
                    backgroundColor: editMode.day === schedule.day && editMode.shift === shift ? '#f0f0f0' : 'inherit',
                  }}
                >
